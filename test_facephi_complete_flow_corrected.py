#!/usr/bin/env python3
"""
Script para probar el flujo completo CORREGIDO de Facephi:
1. documentValidation/v2/start (subir imágenes) → scanReference
2. Esperar 30 segundos
3. documentValidation/v2/status (obtener tokens)
4. extractDocumentDataWeb (procesar con tokens)
"""
...
def test_facephi_complete_flow_corrected():
    """Probar el flujo completo CORREGIDO de Facephi"""
    
    print("🔍 PROBANDO FLUJO COMPLETO CORREGIDO DE FACEPHI")
    print("=" * 60)
    ...
    image_path = "test_document_small.jpg"
    if not os.path.exists(image_path):
        print(f"❌ Error: No se encuentra la imagen {image_path}")
        print("   Ejecuta primero: python create_small_test_image.py")
        return
    ...
    try:
        ...
        start_data = {
            ...
            "documentBackRawImage": image_base64,  # En producción, serían diferentes
        ...
        if start_response.status_code == 200:
            ...
            scan_reference = start_result.get('scanReference')
            if not scan_reference:
                print("   ❌ No se encontró 'scanReference' en la respuesta")
                return
            
            print(f"   🔑 ScanReference obtenido: {scan_reference}")
            print()
            
            # PASO 3: Esperar 30 segundos
            print("🔄 PASO 3: Esperando 30 segundos...")
            ...
            # PASO 4: Llamar a /status
            print("🔄 PASO 4: Llamando a documentValidation/v2/status...")
            
            # Probar diferentes métodos de autenticación para /status
            status_headers = {
                'x-api-key': api_key,
                'Content-Type': 'application/json'
            }
            ...
            status_response = requests.post(
                f"{base_url}/verify/documentValidation/v2/status",
            ...
            if status_response.status_code == 200:
                ...
                print(f"   ✅ ¡ÉXITO en status!")
                print(f"   📄 Respuesta JSON:")
                print(json.dumps(status_result, indent=2, ensure_ascii=False))
                
                # Extraer tokens de la respuesta del status
                token_front = None
                token_back = None
                
                if 'serviceDocument' in status_result:
                    service_doc = status_result['serviceDocument']
                    token_front = service_doc.get('FRONTSIDE', {}).get('TOKEN')
                    token_back = service_doc.get('BACKSIDE', {}).get('TOKEN')
                
                print(f"   🔑 Tokens extraídos del status:")
                print(f"      Front Token: {token_front[:20] + '...' if token_front else 'null'}")
                print(f"      Back Token: {token_back[:20] + '...' if token_back else 'null'}")
                print()
                
                # PASO 5: Llamar a extractDocumentDataWeb con los tokens
                if token_front and token_back:
                    print("🔄 PASO 5: Llamando a extractDocumentDataWeb...")
                    ...
                    print("   ❌ No se pudieron extraer los tokens del status")
            else:
                print(f"   ❌ Error en status: {status_response.status_code}")
                print(f"   📄 Mensaje: {status_response.text}")
        else:
...
if __name__ == "__main__":
    test_facephi_complete_flow_corrected()
